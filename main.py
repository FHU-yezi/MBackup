...
from db_manager import (backup_collection, command_get_dir_size_in_MB,
                        command_remove_dir, get_collection_names_list,
                        get_db_names_list)
...
def full_backup_job() -> None:
    ...
    for db_name in db_names:
        ...
        for collection_name in collection_names:
            backup_collection(config.db.host, config.db.port, config.output_dir,
    ...
    send_task_success_card(config.source, "全量备份", databases_count,
                           collections_count, disk_cost)
